...
class image_process:
    ...
    def image_sub_callback(self, data):
        ...
        ############################## TEST
        # 定义类
        green = get_specific_color(self.img_src, hsv_th=[[25,100,50], [50,255,255]]) # [50,100,50], [80,255,255]
        ...
        if len(approx) == 4:
            if approx[0][0][0] > approx[2][0][0]:# 0 1 2 3
                region.data.append(approx[0][0][0])
                region.data.append(approx[0][0][1])
                region.data.append(approx[1][0][0])
                region.data.append(approx[1][0][1])
                region.data.append(approx[2][0][0])
                region.data.append(approx[2][0][1])
                region.data.append(approx[3][0][0])
                region.data.append(approx[3][0][1])
            else:# 3 0 1 2
                region.data.append(approx[3][0][0])
                region.data.append(approx[3][0][1])
                region.data.append(approx[0][0][0])
        ...
        # box = np.int0(box)
        # 画出来
        cv2.drawContours(self.res_img, cnt, -1, (255, 0, 0), 1)
        self.approx_img = self.res_img.copy()  # 多边形角点图像
        if len(approx) >0: cv2.drawContours(self.approx_img, approx, -1, (255, 255, 255), 10)
        cv2.imshow('res_image', self.approx_img)
