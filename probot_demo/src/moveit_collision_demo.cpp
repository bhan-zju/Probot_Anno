...
/*
为增加问题的复杂性，接下来将在机械臂的末端去附着一个障碍物体，使之成为机械臂的一部分
*/
    // 声明一个附着物体attached_object，附着的物体的id是box，以tool0坐标为基准
    moveit_msgs::AttachedCollisionObject attached_object;
    attached_object.link_name = "tool0";   //该物体将附着在tool0这个link上
    attached_object.object.header.frame_id = "tool0";    //该物体位置的参考坐标
    ...
    //注意，这时的attached_object.object还是一个普通障碍物，加入了场景，但尚未附着到机械臂上
    planning_scene.world.collision_objects.push_back(add_object);
    planning_scene.world.collision_objects.push_back(attached_object.object);
    planning_scene.is_diff = true;
    planning_scene_diff_publisher.publish(planning_scene);    //这时两个物体都已经在场景中了

    // 声明去附着属性的物体，即将从场景中删掉该物体，而添加到机械臂上
    moveit_msgs::CollisionObject remove_box_object;
    remove_box_object.id = "box";   //即将去除掉附着属性的物体的id是box
    remove_box_object.header.frame_id = "tool0";
    remove_box_object.operation = remove_box_object.REMOVE;   //对该物体的操作是REMOVE

    // 将物体附着到机器人上，并从场景中删除
    planning_scene.world.collision_objects.clear();    //由于物体都以加入场景，这个栈区可以清空了
    planning_scene.world.collision_objects.push_back(remove_box_object);
    planning_scene.robot_state.attached_collision_objects.push_back(attached_object);   //将附着物体附着在机械臂
    ...
    remove_cylinder_object.operation = remove_box_object.REMOVE;   //?怎么是remove_box_object?错了吧？
