...
{
	//
	this->curr_img = img;   //cv::Mat型变量curr_img
	//获得图像中心坐标
	img_centre_x_ = img.rows / 2;
	img_centre_y_ = img.cols / 2;

	cv::Rect tablePos;    //定义一个矩形tablePos

	detectTable(tablePos);   //在图像中找到矩形桌面并计算每毫米的像素数pixel_per_mm
...
void VisionManager::detectTable(cv::Rect &tablePos)
{
	...
	cv::Mat BGR[3];
	cv::Mat image = curr_img.clone();    //定义一个图像副本image
	split(image, BGR);
